		...
		public void UpdateSkeleton(SkeletonTransforms skeleton)
		{
			if (Initialized && Player != null && Player.Status == EPlayerStatus.Calibrated)
			{
				...
				if (ShowAvatarOnNextUpdate)
				{
					...
					if (ChairInstance)
					{
						ChairInstance.SetActive(true);

						var chairConfig = ChairInstance.GetComponent<ChairConfig>();
						if (chairConfig)
						{
							var chairConfigSE = chairConfig as BaseSeatedExperienceChairConfig;
							if (chairConfigSE)
                            {
								var skeletonConfig = SharedDataController.Instance.FindSharedDataById<SkeletonConfig>(skeleton.SkeletonId);
								chairConfigSE.AssignPlayer(skeletonConfig);
								//chairConfigSE.EstimateChairRootTransform();
							}	
						}
					}

...
public class TileMakerSphere : MonoBehaviour
{
...
//    public Material reverseMaterial;
    ...
    public void LoadTextures(Texture heightTexture, Texture colorTexture, float scale = 1)
    {
        ...
        if (heightTexture.width > heightTexture.height) {
            transform.localScale = new Vector3(scale, heightTexture.height * scale / heightTexture.width, scale);
        }
        else
        {
    ...
    {
        triangles = new int[2 * (tileSize - 1) * (tileSize - 1) * 3];//last *2 is added for the wall
        vertices = new Vector3[tileSize * tileSize + tileSize * tileSize];
        normals = new Vector3[tileSize * tileSize + tileSize * tileSize];
        for (int j = 0; j < tileSize; j++)
        {
            for (int i = 0; i < tileSize; i++)
            {   float theta = Mathf.Lerp(0,Mathf.PI*2, scale * i / (tileSize - 1.0f));
                float phi = Mathf.Lerp(-Mathf.PI/2,Mathf.PI/2, scale * j / (tileSize - 1.0f));
                float x = Mathf.Sin(theta) * Mathf.Cos(phi);
                float y = Mathf.Sin(theta) * Mathf.Sin(phi);
                float z = Mathf.Cos(phi);
                vertices[j * tileSize + i] = new Vector3(x, y, z);
                normals[j * tileSize + i] = new Vector3(x,y,z)/scale;
            }
        }
        ...
        //        vertices[tileSize * tileSize + k * tileSize + i] = new Vector3((i / (tileSize - 1.0f) - 0.5f) * scale, jLoc, (k / (tileSize - 1.0f)-0.5f)*5*scale);
        //        normals[tileSize * tileSize + k * tileSize + i] = Vector3.forward;
        //    }
        //}
        ...
        GameObject MakeTile(float x, float y, float width, float height, string label = "Tile")
    {
        ...
        for (int j = 0; j < tileSize; j++)
        {
            ...
            {
                ;
                float theta = Mathf.Lerp(0, 2*Mathf.PI, x + i / (tileSize - 1.0f) * width);
                float phi = Mathf.Lerp(0, Mathf.PI, y + j / (tileSize - 1.0f) * height);
                float xx = Mathf.Sin(phi) * Mathf.Cos(theta);
                float yy = Mathf.Sin(phi) * Mathf.Sin(theta);
                float zz = Mathf.Cos(phi);
                vertices[j * tileSize + i] = new Vector3(xx, zz, yy);
                normals[j * tileSize + i] = (new Vector3(xx, zz, yy)) ;
            }
        }
        ...
        if (newUV == null)
        {
            newUV = new Vector2[tileSize * tileSize];
        }
    ...
    void Awake()
    {
        ...
        for (int j = 0; j < numTiles; j++)
        {
            for (int i = 0; i < numTiles; i++)
            {
                ...
                tile.transform.localPosition = Vector3.zero;// new Vector3((i + 0.5f) / numTiles - 0.5f, (j + 0.5f) / numTiles - 0.5f, 0);

...
public class FlockChild : MonoBehaviour
{
    ...
    protected float _soarTimer;
    protected bool _instantiated;
    protected static int _updateNextSeed = 0;
    protected int _updateSeed = -1;
    //[HideInInspector] 
    ...
    private NetworkActivated networkActivated;
    ...
    public void Start()
    {
        ...
        networkActivated = GetComponent<NetworkActivated>();
    }
    ...
    public bool Avoidance()
    {
        ...
        if (Physics.Raycast(_thisT.position, -_modelT.forward, out hit, _avoidDistance * 2f, _spawner._avoidanceMask))
        {
            if (Vector3.Distance(_modelT.right, hit.normal) < Vector3.Distance(-_modelT.right, hit.normal))
        ...
        else if (Physics.Raycast(_thisT.position, _modelT.forward, out hit, _avoidDistance, _spawner._avoidanceMask))
        {
            //Rotate here instead of changing position
            if (Vector3.Distance(_modelT.right, hit.normal) < Vector3.Distance(-_modelT.right, hit.normal))
                rotE.y -= _spawner._birdAvoidHorizontalForce * _spawner._newDelta * _damping;
            else
                rotE.y += _spawner._birdAvoidHorizontalForce * _spawner._newDelta * _damping;
        ...
        if (Physics.Raycast(_thisT.position, _modelT.right, out hit, _avoidDistance, _spawner._avoidanceMask))
        {
            rotE.y += _spawner._birdAvoidHorizontalForce * _spawner._newDelta * _damping;
            rot.eulerAngles = rotE;
            _thisT.rotation = rot;

            pos += hit.normal * _spawner._birdAvoidHorizontalForce * _spawner._newDelta * .01f;
            _thisT.position = pos;
            r = true;
        }
        else if (Physics.Raycast(_thisT.position, -_modelT.right, out hit, _avoidDistance, _spawner._avoidanceMask))
        {
            rotE.y -= _spawner._birdAvoidHorizontalForce * _spawner._newDelta * _damping;
            rot.eulerAngles = rotE;
            _thisT.rotation = rot;
    ...
    public void Flap()
    {
        if (_move)
        {
            if (this._model != null && !System.String.IsNullOrEmpty(_spawner._flapAnimation)) _model.GetComponent<Animation>().CrossFade(_spawner._flapAnimation, .5f);
            _soar = false;
            animationSpeed();
    ...
    public void Soar()
    {
        if (_move)
        {
            if (!System.String.IsNullOrEmpty(_spawner._soarAnimation))

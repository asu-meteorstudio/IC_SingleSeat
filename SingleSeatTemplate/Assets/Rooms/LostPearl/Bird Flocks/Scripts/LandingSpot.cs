...
public class LandingSpot : MonoBehaviour {
    ...
    public bool _gotcha;
    public bool hideAllMeshesAtStart = false;
    ...
    public void Start() {
        ...
        if (hideAllMeshesAtStart)
        {
            Invoke("HideMeshes", 1.0f);
        }
        ...
        if (_controller._flock.gameObject.activeInHierarchy && landing && (landingChild != null))
        {
            ...
            if (distance < 5 && distance > .5f) {
                if(_controller._soarLand){
                    if(!System.String.IsNullOrEmpty(landingChild._spawner._soarAnimation))
                	    landingChild._model.GetComponent<Animation>().CrossFade(landingChild._spawner._soarAnimation, .5f);
                	if (distance < 2)
                        if (!System.String.IsNullOrEmpty(landingChild._spawner._flapAnimation))
                ...
                if (distance < _controller._snapLandDistance && !_idle) {
                    _idle = true;
                    if (!System.String.IsNullOrEmpty(landingChild._spawner._idleAnimation))
    ...
    public void ReleaseFlockChild() {
        ...
        if (_controller._flock.gameObject.activeInHierarchy && (landingChild != null)) {
            ...
            landingChild.Flap();     	
            landingChild._wayPoint = new Vector3(landingChild._wayPoint.x, _thisT.position.y, landingChild._wayPoint.z);
    ...
    public void HideMeshes()
    {
        ...
        SkinnedMeshRenderer[] skinnedMeshRenderers = landingChild.GetComponentsInChildren<SkinnedMeshRenderer>();
    ...
    public void ShowOrigamiMesh()
    {
        SkinnedMeshRenderer[] skinnedMeshRenderers = landingChild.GetComponentsInChildren<SkinnedMeshRenderer>(true);
        foreach (SkinnedMeshRenderer skinnedMeshRenderer in skinnedMeshRenderers)
        {
    ...
    public void ShowSphereMesh()
    {
        this.GetComponent<MeshRenderer>().enabled = true;
       
    }

    public void StartAnimationOnFlockChild()
    {
        Debug.Log("Hit the orb bird should come out");
        this.GetComponent<MeshRenderer>().enabled = false;
        ShowOrigamiMesh();
        Animator[] anims = landingChild.GetComponentsInChildren<Animator>();

...
namespace JBooth.MegaSplat
{
   ...
   public class TextureArrayConfigEditor : Editor
   {
      ...
      string GetNameRoot(TextureArrayConfig cfg, TextureArrayConfig.TextureEntry te)
      {
         ...
         if (name.EndsWith(cfg.extDiffuse.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extDiffuse.ToLower()));
         }
         else if (name.EndsWith(cfg.extNorm.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extNorm.ToLower()));
         }
         else if (name.EndsWith(cfg.extSmoothness.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extSmoothness.ToLower()));
         }
         else if (name.EndsWith(cfg.extAO.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extAO.ToLower()));
         }
         else if (name.EndsWith(cfg.extMetal.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extMetal.ToLower()));
         }
         else if (name.EndsWith(cfg.extHeight.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extHeight.ToLower()));
         }
         else if (name.EndsWith(cfg.extEmiss.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extEmiss.ToLower()));
         }
         else if (name.EndsWith(cfg.extAlpha.ToLower()))
         {
            name = name.Substring(0, name.LastIndexOf(cfg.extAlpha.ToLower()));
         }
         return name;
      }
      ...
      void AssignTexture(TextureArrayConfig cfg, Texture2D tex)
      {
         ...
         name = name.ToLower();
         if (name.EndsWith(cfg.extHeight.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extHeight.ToLower())).ToLower();
            bool found = false;
         ...
         if (name.EndsWith(cfg.extNorm.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extNorm.ToLower())).ToLower();
            bool found = false;
            for (int i = 0; i < cfg.sourceTextures.Count; ++i)
            {
         ...
         if (name.EndsWith(cfg.extAO.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extAO.ToLower())).ToLower();
            bool found = false;
            for (int i = 0; i < cfg.sourceTextures.Count; ++i)
            {
         ...
         if (name.EndsWith(cfg.extSmoothness.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extSmoothness.ToLower())).ToLower();
            bool found = false;
            for (int i = 0; i < cfg.sourceTextures.Count; ++i)
            {
         ...
         if (name.EndsWith(cfg.extEmiss.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extEmiss.ToLower())).ToLower();
            bool found = false;
            for (int i = 0; i < cfg.sourceTextures.Count; ++i)
            {
         ...
         if (name.EndsWith(cfg.extAlpha.ToLower()))
         {
            var compName = name.Substring(0, name.LastIndexOf(cfg.extAlpha.ToLower())).ToLower();
            bool found = false;
            for (int i = 0; i < cfg.sourceTextures.Count; ++i)
            {
      ...
      public static void CompileConfig(TextureArrayConfig cfg)
      {
         ...
         for (int i = 0; i < cfg.sourceTextures.Count; ++i)
         {
            try
            {
               ...
               {
                  EditorUtility.CompressTexture(normalSAOTex, GetTextureFormat(), tq);
               }

               if (cfg.diffuseCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(diffuseHeightTex, GetTextureFormat(), tq);
               }

               if (cfg.emisCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(emisTex, GetTextureFormat(), tq);
               }

               if (cfg.alphaCompression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(alphaTex, GetTextureFormat(), tq);
               }

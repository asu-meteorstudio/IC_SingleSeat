...
#pragma warning restore 618
        ...
        */
        ...
        public void Interpolate(float f, out Vector3 pos, out Vector3 up)
        {
            float frag;
            int idx = GetFIndex(f, out frag);
            pos = Vector3.LerpUnclamped(Position[idx], Position[idx + 1], frag);
    ...
    public class CGPath : CGShape
    {
        ...
        public void Interpolate(float f, out Vector3 pos, out Vector3 dir, out Vector3 up)
        {
            ...
            int idx = GetFIndex(f, out frag);
            pos = Vector3.LerpUnclamped(Position[idx], Position[idx + 1], frag);
            dir = Vector3.SlerpUnclamped(Direction[idx], Direction[idx + 1], frag);
            up = Vector3.SlerpUnclamped(Normal[idx], Normal[idx + 1], frag);
        }
    ...
    public class CGVMesh : CGBounds
    {
        ...
        public static CGVMesh Get(CGVMesh data, CGVolume source, IntRegion subset, bool addUV, bool reverseNormals)
        {
            ...
            else
            {
                ...
                //data.SubMeshes = new CGVSubMesh[0];//TODO BUG? why is this commented?
            ...
            if (reverseNormals)
                //TODO OPTIM merge loop with normals copy
    ...
    public class CGDataRequestRasterization : CGDataRequestParameter
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (O == null)
                return false;
            return (Start == O.Start && RasterizedRelativeLength == O.RasterizedRelativeLength && Resolution == O.Resolution && SplineAbsoluteLength == O.SplineAbsoluteLength && AngleThreshold == O.AngleThreshold && Mode == O.Mode);
        }

        public override int GetHashCode()
        {
            return new { A = Start, B = RasterizedRelativeLength, C = Resolution, D = AngleThreshold, E = Mode, F = SplineAbsoluteLength }.GetHashCode();
        }
    }

...
public static class MultiSceneSetupMenu
{
	...
	static void SaveCurrentSceneSetup(string assetPath)
	{
		...
		loader.Setups = new Utilities.SceneSetup[setups.Length];
		for (var i = 0; i < setups.Length; i++)
		{
			loader.Setups [i] = new Utilities.SceneSetup ();
			loader.Setups [i].isActive = setups [i].isActive;
			loader.Setups [i].isLoaded = setups [i].isLoaded;
            loader.Setups[i].SceneReference = new SceneReference();
            //loader.Setups[i].SceneReference.ScenePath = EditorSceneManager.GetSceneByPath(setups[i].path);
            loader.Setups[i].SceneReference.ScenePath = setups[i].path;
        }
	...
	public static void RestoreSceneSetup()
	{
		...
		var setups = new SceneSetup[loader.Setups.Length];
		for (var i = 0; i < loader.Setups.Length; i++)
		{
			setups [i] = new SceneSetup ();
			setups [i].isActive = loader.Setups [i].isActive; 
			setups [i].isLoaded = loader.Setups [i].isLoaded;
			setups [i].path = loader.Setups [i].SceneReference.ScenePath;
		}

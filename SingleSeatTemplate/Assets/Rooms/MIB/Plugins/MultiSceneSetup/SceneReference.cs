...
public class SceneReference : ISerializationCallbackReceiver
{
    ...
    //HACK + TODO -> should probably just cache / serialize this along with the path
    string sceneName;
    public string SceneName
    {
        get
        {
            ...
            {
                if(!string.IsNullOrEmpty(ScenePath) && ScenePath.Length > 0)
                {
                    var startIndex = ScenePath.LastIndexOf("/") + 1;
                    return sceneName = ScenePath.Substring(startIndex, ScenePath.Length - startIndex - 6);
                }
                else
                {
                    return "";

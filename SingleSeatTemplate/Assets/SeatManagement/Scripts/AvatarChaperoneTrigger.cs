...
{
    public class AvatarChaperoneTrigger : NetworkSyncedBehaviour
    {
        ...
        {
            var avatarChaperone = other.GetComponentInParent<AvatarChaperone>();
            if (avatarChaperone)
            {
                var bodyPart = other.GetComponent<AvatarChaperoneBodyPart>();
                if (bodyPart)
                {
                    //Send network message
                    SendTriggerEvent(bodyPart.BodyPart, avatarChaperone.SourceAvatarController.PlayerId, true);
                }
            }
        }
        ...
        void OnTriggerExit(Collider other)
        {
            ...
            if (avatarChaperone)
            {
                ...
                {
                    //Send network message
                    SendTriggerEvent(bodyPart.BodyPart, avatarChaperone.SourceAvatarController.PlayerId, false);
                }
            }
        }
        ...
        private void NetworkMessage_AvatarTrigger(Location.Messages.AvatarTrigger args)
        {
            ...
            {
                if (args.BodyPart == EAvatarBodyPart.Head)
                    Debug.Log("[" + Time.frameCount + "] - Received avatarChaperoneTrigger event: " + gameObject.name + " - " + args.BodyPart + " - " + args.PlayerId + " - " + args.State);

                var player = GameController.Instance.GetPlayerByPlayerId(args.PlayerId);
                if (player != null && player.AvatarController)
                {
                    //Check specific part
                    switch (args.BodyPart)
                    {
                        case EAvatarBodyPart.LeftFoot:
                            if (args.State)
                                OnLeftFootEnter.Invoke(player.AvatarController);
                            else
                                OnLeftFootExit.Invoke(player.AvatarController);

                            break;

                        case EAvatarBodyPart.RightFoot:
                            if (args.State)
                                OnRightFootEnter.Invoke(player.AvatarController);
                            else
                                OnRightFootExit.Invoke(player.AvatarController);

                            break;

                        case EAvatarBodyPart.LeftHand:
                            if (args.State)
                                OnLeftHandEnter.Invoke(player.AvatarController);
                            else
                                OnLeftHandExit.Invoke(player.AvatarController);

                            break;

                        case EAvatarBodyPart.RightHand:
                            if (args.State)
                                OnRightHandEnter.Invoke(player.AvatarController);
                            else
                                OnRightHandExit.Invoke(player.AvatarController);

                            break;

                        case EAvatarBodyPart.Head:
                            if (args.State)
                                OnHeadEnter.Invoke(player.AvatarController);
                            else
                                OnHeadExit.Invoke(player.AvatarController);
                            break;
        ...
        private void SendTriggerEvent(EAvatarBodyPart bodyPart, Guid playerId, bool state)
        {
            ...
            {
                if (bodyPart == EAvatarBodyPart.Head) Debug.Log("[" + Time.frameCount + "] - Send avatarChaperoneTrigger event: " + gameObject.name + " - " + bodyPart + " - " + playerId + " - " + state);
